...
export async function createRoom(formData: FormData) {
  ...
  const { error: playerError } = await adminAuthClient.from("players").insert({
    room_id: room.id,
...
export async function createTransaction(
  ...
  if (
    ["player_to_player", "player_to_bank", "pot_in"].includes(type) &&
    fromPlayerId
  ) {
    ...
    if (player && player.current_balance < amount) {
      throw new Error(
        `Insufficient funds. You only have $${player.current_balance}.`
      );
    }
  }
...
export async function revertTransaction(
  ...
  const { data: originalTransaction, error: fetchError } = await adminAuthClient
    .from("transactions")
    .select("*")
    .eq("id", transactionId)
    .single();
  ...
  // 2. Create a reversal transaction with opposite from/to and reversal type
  const reversalDescription = `REVERSAL: ${
    originalTransaction.description || "Transaction"
  }`;

  // Swap from and to for reversal
  const reversedFromPlayerId = originalTransaction.to_player_id;
  const reversedToPlayerId = originalTransaction.from_player_id;

  // 3. Call RPC to create reversal transaction
  const { error } = await adminAuthClient.rpc("perform_transaction", {
    p_room_id: originalTransaction.room_id,
    p_from_player_id: reversedFromPlayerId || null,
    p_to_player_id: reversedToPlayerId || null,
    p_amount: originalTransaction.amount,
    p_type: "reversal",
    p_description: reversalDescription,
  });

  if (error) throw new Error(error.message);

  // 4. Store the original transaction ID in metadata for tracking
  const { error: updateError } = await adminAuthClient
    ...
    .eq("room_id", originalTransaction.room_id)
    .order("created_at", { ascending: false })
    .limit(1);

  if (updateError) {
    console.error("Failed to update reversal metadata:", updateError);
  }

  if (roomCode) {
    revalidatePath(`/room/${roomCode}/game`, "page");
  }
}
...
export async function rollDice(
  ...
  const { error } = await adminAuthClient.from("game_events").insert({
    room_id: roomId,
    player_id: playerId,
...
export async function createLoan(
  ...
  const { error: loanError } = await adminAuthClient.from("loans").insert({
    room_id: roomId,
    player_id: playerId,
    amount: amount,
    description: description,
...
export async function repayLoan(
  ...
  await createTransaction(
    ...
    "player_to_bank",
    amount,
    `Loan Repayment: ${loan.description}`,
    loan.player_id,
    undefined
  );
...
export async function getTransactions(roomId: string) {
  ...
  const { data } = await adminAuthClient
    .from("transactions")
    .select(
      "*, from_player:players!from_player_id(nickname), to_player:players!to_player_id(nickname)"
    )
    .eq("room_id", roomId)
    .order("created_at", { ascending: false })
    .limit(50);
